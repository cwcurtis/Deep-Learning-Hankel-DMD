...
def testing_run(dataloader, model, loss_fn, window_cap):
    ...
    recon_val_losses = np.array([])
    pred_val_losses = np.array([])
    dmd_val_losses = np.array([])
        ...
        with torch.no_grad():
            for (cnt, batch) in enumerate(epoch):
                if cnt < window_cap:
                    ...
                    total_losses = np.append(dmd_val_losses, losses[0].item())
                    recon_val_losses = np.append(dmd_val_losses, losses[1].item())
                    dmd_val_losses = np.append(dmd_val_losses, losses[2].item())
                    pred_val_losses = np.append(dmd_val_losses, losses[3].item())
                else:
                    break
    avg_total_loss = np.mean(total_losses)
    avg_recon_loss = np.mean(recon_val_losses)
    avg_dmd_loss = np.mean(dmd_val_losses)
    avg_pred_loss = np.mean(pred_val_losses)
